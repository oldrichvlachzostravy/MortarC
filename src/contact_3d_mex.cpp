/* contact_3d_mex.c
Syntax:  [N,T1,T2,gn,gt1,gt2,TT2N,supports,master_nodes_indices] = contact_3d_mex(i,fem, master_els, slave_els, friction);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if ( nrhs != 5 ) /* Check the number of arguments */
		mexErrMsgTxt("contact_3d_mex: 5 input arguments needed, check the syntax");
	else if( nlhs > 4 )
		mexErrMsgTxt("contact_3d_mex: Too many output arguments");
	...
	if ( mxGetM(I)==1 && mxGetN(I)==1 && mxGetClassID(I)==mxDOUBLE_CLASS )
	{
		...
		mexErrMsgTxt("contact_3d_mex: 1st input argument ( i ) should be mxDOUBLE_CLASS");
	}
	...
	if ( mxGetM(FEM)==1 && mxGetN(FEM)==1 && strcmp(mxGetClassName(FEM),mxFEMFEM_CLASS)==0 )
	{
		...
		mexErrMsgTxt("contact_3d_mex: 2nd input argument ( fem ) should be Classes.valueClasses.fem.Fem");
	}
	...
	if ( mxGetClassID(MASTER_ELS)==mxDOUBLE_CLASS )
	{
		...
		mexErrMsgTxt("contact_3d_mex: 3rd input argument ( master_els ) should be mxDOUBLE_CLASS");
	}
	...
	if ( mxGetClassID(SLAVE_ELS)==mxDOUBLE_CLASS )
	{
		...
		mexErrMsgTxt("contact_3d_mex: 4th input argument ( slave_els ) should be mxDOUBLE_CLASS");
	}
	...
	if ( mxGetM(FRICTION)==1 && mxGetN(FRICTION)==2 && mxGetClassID(FRICTION)==mxDOUBLE_CLASS )
	{
		...
		mexErrMsgTxt("contact_3d_mex: 5th input argument ( friction ) should be 1x2 mxDOUBLE_CLASS");
	}

	/* coordinates0 */
	tmp_mxArray_ptr = mxGetProperty( FEM, (unsigned long int)0, const_string_mesh);
	if ( tmp_mxArray_ptr == NULL )
		mexErrMsgTxt("contact_3d_mex: 2nd input argument ( fem ) do not have the '.mesh' property\n");
	if ( mxGetM(tmp_mxArray_ptr)!=1 || mxGetN(tmp_mxArray_ptr)!=1 || strcmp(mxGetClassName(tmp_mxArray_ptr),mxFEMMESH_CLASS)!=0 )
		mexErrMsgTxt("contact_3d_mex: 2nd input argument ( fem ) have the '.mesh' of other than 1 x 1 Classes.optionsClasses.Matsol_options class\n");
	tmp_mxArray_ptr = mxGetProperty( tmp_mxArray_ptr, (unsigned long int)0, const_string_coordinates0);
	if ( tmp_mxArray_ptr == NULL )
		mexErrMsgTxt( "contact_3d_mex: 2nd input argument ( fem  ) do not have the 'mesh.coordinates0' property\n");
	...
	{
		mexPrintf("contact_3d_mex: 2nd input argument ( fem ) fem.mesh.coordinates0 should be mxDOUBLE_CLASS");
	}

	/* problem_name, example_root */
	tmp_mxArray_ptr = mxGetProperty( FEM, (unsigned long int)0, const_string_options);
	if ( tmp_mxArray_ptr == NULL )
    	mexErrMsgTxt("contact_3d_mex: 2nd input argument ( fem ) do not have the '.options' property\n");
	if ( mxGetM(tmp_mxArray_ptr)!=1 || mxGetN(tmp_mxArray_ptr)!=1 || strcmp(mxGetClassName(tmp_mxArray_ptr),mxOPTIONSMATSOLOPTIONS_CLASS)!=0 )
		mexErrMsgTxt("contact_3d_mex: 2nd input argument ( fem ) have the '.options' of other than 1 x 1 Classes.optionsClasses.Matsol_options class\n");
	...
	if ( mxGetM(tmp_mxArray_ptr)!=1 || mxGetN(tmp_mxArray_ptr)!=1 || strcmp(mxGetClassName(tmp_mxArray_ptr),mxOPTIONSGENERALOPTIONS_CLASS)!=0 )
		mexErrMsgTxt("contact_3d_mex: 2nd input argument ( fem ) have the '.options' of other than 1 x 1 Classes.optionsClasses.General_options class\n");
	tmp1_mxArray_ptr = mxGetProperty( tmp_mxArray_ptr, (unsigned long int)0, const_string_problem_name);
	if ( tmp1_mxArray_ptr == NULL )
		mexErrMsgTxt( "contact_3d_mex: 2nd input argument ( fem  ) do not have the options.general.problem_name property\n");
	if ( mxIsChar(tmp1_mxArray_ptr) && mxGetM(tmp1_mxArray_ptr)==1 )
	{
		...
		if (tmpint2 != 0)
			mexErrMsgTxt("contact_3d_mex: Not enough space. The string fem.options.general.problem_name is too long.\n");
		mexPrintf("contact_3d_mex: fem.options.general.problem_name is %s\n",problem_name);
	}
	else
	{
		mexPrintf("contact_3d_mex: 2nd input argument ( fem ) fem.options.general.problem_name should be 1 x length mxCHAR_CLASS\n");
	}
	...
	if ( tmp1_mxArray_ptr == NULL || mxGetM(tmp1_mxArray_ptr)!=1)
		mexErrMsgTxt( "contact_3d_mex: 2nd input argument ( fem  ) do not have the options.general.example_root property\n");
	if ( mxIsChar(tmp1_mxArray_ptr) && mxGetM(tmp1_mxArray_ptr)==1 )
	{
		tmpint1= mxGetN(tmp1_mxArray_ptr)+1;
		...
		if (tmpint2 != 0)
			mexErrMsgTxt("contact_3d_mex: Not enough space. The string fem.options.general.example_root is too long.\n");
		//mexPrintf("contact_3d_mex: fem.options.general.example_root is %s\n",example_root);
	}
	else
	{
		mexPrintf("contact_3d_mex: 2nd input argument ( fem ) fem.options.general.example_root should be 1 x length mxCHAR_CLASS\n");
	}
	...
	if (DEBUG_OUTPUTS)
	{
		mexPrintf("contact_3d_mex: reading boundaries  ... done\n");
	}
